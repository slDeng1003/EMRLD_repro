...
import time
import argparse
from envs.two_wheel import TwoWheelEnv
...
def maml_a2c_loss(train_episodes, learner, baseline, gamma, tau, task_data, w_a2c, w_bc):
    ...
    demo_states = torch.tensor(task_data['state']).float().to(device)
    demo_actions = torch.tensor(task_data['action']).float().to(device)
    demo_adv = torch.ones((demo_states.shape[0], 1)).to(device)
...
def main(
    ...
    print("Running EMRLD-WS for TwoWheel Locomotion")
    ...
    def make_env():
        env = TwoWheelEnv(tresh=tresh, r_tresh=r_tresh, is_sparse=True)
    ...
    for iteration in range(num_iterations):
        ...
        for task_config in tqdm(traj_data, leave=False, desc='Data'): 
            ...
            ##BC Adapt Step##
            train_episodes = None
            # task_replay.append(train_episodes) # Cause 'NoneType' Error in r.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tresh', type=float, default=0.2, metavar='G',
                        help='Goal Treshold')
    parser.add_argument('--r-tresh', type=float, default=0.5, metavar='G',
    ...
    parser.add_argument('--w-bc', type=float, default=1.0, metavar='G',
                        help='w-bc')
    parser.add_argument('--data-path', metavar='G', default='Traj_Data/Good_TW.p',
                        help='path of data')
    ...
    if args.exp_num == 1:
        args.data_path = 'Traj_Data/Good_TW.p'
    elif args.exp_num == 2:
        args.data_path = 'Traj_Data/Bad_TW.p'
        args.w_a2c = 1.0
    else:
