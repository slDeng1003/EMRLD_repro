...
def fast_adapt_a2c(clone, train_episodes, adapt_lr,adapt_a2c_lr, baseline, gamma, tau, task_data, w_a2c, w_bc,
	...
	# ## my check of gradients none or not
	# for grad, variable in zip(gradients, clone.parameters()):
	# 	if grad is not None:
	# 		print(f"gradients=autograd.grad() is NOT None.")
	# 	else:
	# 		print(f"!!! gradients=autograd.grad() is None!")
	# 	if variable.grad is not None:
	# 		print(f"After autograd(), clone.parameters().grad is NOT None.")
	# 	else:
	# 		print(f"!!! After autograd(), clone.parameters().grad is None!")
...
def main(
	...
	for iteration in range(num_iterations):
		...
		for task_config in tqdm(traj_data, leave=False, desc='Data'):  
			...
			print(f"task_data: \n {task_data}")
			## 这里的deepcopy只是编程上方便。因为在iteration-task的双层loop下，clone不方便存储，后面meta-update不好调用。
			## 另外，也方便并行。
			## 在meta-update步时，需要clone=clone_module(policy)传导梯度，即policy <- apdated_clone <- clone
