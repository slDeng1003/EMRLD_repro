...
def main(
    ...
    def make_env():
        ...
        env = HalfCheetahForwardBackwardEnv(sparse_val = sparse_val)
    ...
    for iteration in range(num_iterations):
        ...
        # Line-search
            # Update policy with different stepsize (lr), if new lr is better, then update.
        for ls_step in range(ls_max_steps):
            ...
            if new_loss < old_loss and kl < max_kl:
                # if the new lr is better, then use it to update the original policy.
                # Update policy's parameter by adding lr*grad (informally) , same as p += lr*grad
